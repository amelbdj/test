...
agent_communication:
  ...
  - task: "Push Token Registration"
    ...
    status_history:
      - working: true
        agent: "testing"
        comment: "POST /api/push-token endpoint working perfectly - accepts push token and stores it for the authenticated user. Returns success message as expected."
  ...
  - task: "Streak Details"
    ...
    status_history:
      - working: true
        agent: "testing"
        comment: "GET /api/streak/details endpoint working correctly - returns all required fields (current_streak, max_streak, streak_freezes, milestones, next_milestone, days_active_this_week, is_at_risk). Provides comprehensive streak information with proper data types."
